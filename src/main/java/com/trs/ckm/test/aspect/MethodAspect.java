...
public class MethodAspect {
	...
	/** 当参数长度超过此值时, 裁减掉后边的文字 */
	private final static int PARAMETER_CUT_SIZE = 30; 
	...
	static String buildLogSentences(String method, Object[] args) {
		...
		for(int i=0; i<args.length; i++) {
			...
			if(i < args.length - 1)
				logSentence.append(args[i]).append(", ");
		}
		logSentence.append(")");
	...
	public void afterMethod(JoinPoint point, CkmBasicResult result) {
	...
	static class PointCut{
		final static String ALL = "execution(* com.trs.ckm.api.master.TRSCkmRequest.*(..))";
	}
}
